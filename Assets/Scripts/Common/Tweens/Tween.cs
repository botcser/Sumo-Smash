...
namespace Assets.Scripts.Common.Tweens
{
    public class Tween : MonoBehaviour
    {
        ...
        public static Tween Common(Component target, Action<float> action, int type, float from, float to, float duration)
        {
            return Common(target.gameObject, action, type, from, to, duration);
        }

        public static Tween Common(GameObject target, Action<float> action, int type, float from, float to, float duration)
	    {
		    action(from);
